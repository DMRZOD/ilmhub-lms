...
export function renderQaAnswerEmail({
  name,
  title,
  body,
  link,
}: QaAnswerPayload): RenderedEmail {
  const subject = `Savolingizga javob: ${title}`;
...
<html lang="uz">
  <body style="margin:0;padding:24px;background:#f7f7f8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;color:#111;">
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;">
      <tr><td>
        <h1 style="margin:0 0 16px;font-size:22px;">Savolingizga javob keldi</h1>
        <p style="margin:0 0 12px;line-height:1.5;">Assalomu alaykum, ${safeName}!</p>
        <p style="margin:0 0 12px;line-height:1.5;"><strong>${safeTitle}</strong> savolingizga yangi javob berildi.</p>
        ...
        ${link ? `<a href="${link}" style="display:inline-block;background:#111;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;">Javobni ko'rish →</a>` : ''}
  ...
  const text = `Assalomu alaykum, ${name}!\n\n${title} savolingizga yangi javob berildi.\n\n${body}${link ? '\n\n' + link : ''}`;
